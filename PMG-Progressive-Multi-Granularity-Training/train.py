...
# begin modified by nemo
import my_custom_dataset
# end modified by nemo
...
def train(nb_epoch, batch_size, store_name, resume=False, start_epoch=0, model_path=None):
    ...
    #begin modified by nemo
    trainset = dataset.trainset(transform=transform_train)
    #end modified by nemo
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        train_ds = progressBar(enumerate(trainloader), len(trainloader))
        for batch_idx, (inputs, targets) in train_ds:
            idx = batch_idx
...
train(nb_epoch=200,             # number of epoch
      ...
      store_name=f'expiriment_time={time.ctime().replace(" ", "-")}',     # folder for output
