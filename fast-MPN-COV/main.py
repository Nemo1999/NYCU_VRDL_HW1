...
def main():
    ...
    #replace training dataset by custom one
    ...
    """
    train_transforms, val_transforms, evaluate_transforms = preprocess_strategy(args.benchmark)
    train_dataset = my_custom_dataset.trainset(transform=train_transforms, root=args.data)
    ...
    #replace validation dataset by custom one
    """
    ...
    val_dataset = my_custom_dataset.testset(transform=val_transforms, root=args.data)
    val_loader = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True)
    ...
    if evaluate_transforms is not None:
        ...
        evaluate_set = my_custom_dataset.evalset(transform=evaluate_transforms, root=args.data)
        evaluate_loader = torch.utils.data.DataLoader(
            evaluate_set,
...
def evaluate(eval_loader, model, criterion):
    ...
    with torch.no_grad():
      with open(os.path.join(args.modeldir, 'evaluation_results.txt'), "a") as f:
        end = time.time()
